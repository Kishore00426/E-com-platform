    ...
    return (
        <div className="space-y-10 animate-in fade-in duration-700">
            ...
            <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h1 className="text-4xl text-gray-900 tracking-tighter">Product Inventory</h1>
                    <p className="text-gray-500 mt-1 tracking-tight">Manage and track your products, stock, and local uploads.</p>
                </div>
                <div className="flex items-center gap-3">
                    <div className="bg-green-50 border border-green-100 text-green-700 px-6 py-3 rounded-2xl text-sm shadow-sm flex items-center gap-2">
                        <Package className="w-5 h-5" /> {products.length} Items Total
                    </div>
                    <button 
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-100 flex items-center justify-center p-4">
                    ...
                    <section className="relative w-full max-w-4xl bg-white p-10 rounded-[40px] shadow-2xl border border-gray-100 animate-in zoom-in duration-300 max-h-[90vh] overflow-y-auto no-scrollbar">
                        <div className="flex justify-between items-center mb-10">
                            <h2 className="text-3xl text-gray-900 flex items-center gap-3 tracking-tighter">
                                <Plus className="w-8 h-8 text-blue-600" /> List New Product
                            </h2>
                        ...
                        <form onSubmit={handleCreate} className="space-y-8">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                                ...
                                <div className="space-y-3">
                                    ...
                                    <select 
                                        ...
                                        className={`w-full px-6 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-2 focus:ring-blue-500/20 outline-none transition-all shadow-sm ${!newProd.category_id ? 'opacity-50 cursor-not-allowed text-gray-300' : ''}`}
                                        required
                                ...
                                <div className="space-y-3">
                                    ...
                                    <label className="w-full flex items-center gap-3 px-6 py-3.5 bg-blue-50 border border-blue-100 rounded-2xl cursor-pointer hover:bg-blue-100 transition-all border-dashed group">
                                        <Upload className="w-5 h-5 text-blue-600 group-hover:scale-110 transition-transform" />
                                        <span className="text-blue-700 text-sm">{selectedFiles.length > 0 ? `${selectedFiles.length} files selected` : 'Click to Upload Images'}</span>
                            ...
                            <div className="space-y-3">
                                ...
                                <textarea 
                                    ...
                                    className="w-full px-6 py-4 bg-gray-50 border border-gray-100 rounded-3xl focus:ring-2 focus:ring-blue-500/20 outline-none min-h-[140px] transition-all shadow-sm leading-relaxed"
            ...
            {/* Products Table */}
            <div className="bg-white rounded-[40px] shadow-2xl border border-gray-100 overflow-hidden animate-in slide-in-from-bottom-4 duration-1000">
                <table className="w-full text-left ">
                    <thead className="bg-gray-50/50 border-b border-gray-100">
                        <tr>
                            <th className="px-10 py-6 text-sm text-gray-600 uppercase tracking-widest">Product Details</th>
                            <th className="px-10 py-6 text-sm text-gray-600 uppercase tracking-widest">Price</th>
                            <th className="px-10 py-6 text-sm text-gray-600 uppercase tracking-widest">Stock</th>
                            <th className="px-10 py-6 text-sm text-gray-600 uppercase tracking-widest text-right">Actions</th>
                        </tr>
                    </thead>
                            ...
                            <tr key={prod.id} className="hover:bg-blue-50/10 transition-colors group">
                                <td className="px-10 py-8">
                                    <div className="flex items-center gap-6">
                                        ...
                                        <div>
                                            <p className="text-gray-900 text-xl leading-snug">{prod.name}</p>
                                            <div className="flex gap-2 mt-2">
                                                <span className="text-[10px] text-blue-600 px-2 py-0.5 border border-blue-100 rounded-full uppercase">
                                ...
                                <td className="px-10 py-8 text-gray-900 text-lg tracking-tighter">₹{prod.price}</td>
                                <td className="px-10 py-8">
                                    <div className={`flex items-center gap-2 ${prod.stock < 10 ? 'text-red-600' : 'text-green-600'}`}>
                                        <div className={`w-2 h-2 rounded-full ${prod.stock < 10 ? 'bg-red-500 animate-pulse' : 'bg-green-500'}`} />
                                        <span>{prod.stock} Units</span>
                                    </div>
                                </td>
                                <td className="px-10 py-8 text-right space-x-2">
                                    <button className="p-3.5 text-blue-500 hover:bg-blue-50 rounded-2xl transition-all border border-transparent hover:border-blue-100 shadow-sm">
                                        <Edit3 className="w-6 h-6" />
                                    </button>
                                    <button onClick={() => handleDelete(prod.id)} className="p-3.5 text-red-500 hover:bg-red-50 rounded-2xl transition-all border border-transparent hover:border-red-100 shadow-sm">
